class User < ActiveRecord::Base
  ...
  #scope :upcoming_birthdays, lambda { where(:conditions => [birthday.next_birthday.between(Date.current - 1.week, Date.current + 2.weeks)])}
  ...
  def self.upcoming_birthdays
    ...
    find(:all).each do |user|
      if user.next_birthday.between?(Date.current, Date.current + 2.weeks)
        birthdays << user
      end
    end
